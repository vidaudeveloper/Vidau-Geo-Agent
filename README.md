...
或把 **[INSTALL_PROMPT.md](INSTALL_PROMPT.md)** 里的中文版 / English 整段复制给 Cursor / Hermes / Claude Desktop。

**使用指南：** 安装完成后阅读 **[SKILLS_GUIDE.md](SKILLS_GUIDE.md)**（Skills 编排 MCP、8 个 Skill 示例话术、排错）。在线版：[Help Center · Agent Skills](https://geo.vidau.ai/en/help/agent-skills)。
...
```bash
pnpm skills:install          # copies from this repo into ~/.hermes/skills/vidau-geo/
pnpm skills:install --remote # fallback: install via GitHub Contents API
```

**One-liner (production CDN):**
...
**Offline:** download [vidau-geo-skills.zip](https://geo.vidau.ai/skills/vidau-geo-skills.zip) or clone this repo and run `node scripts/install-skills.mjs --force`.
...
保存后执行 `/reload-mcp` 或重启 Agent。

### 第 2 步 — 安装 Skills

把 [docs/SETUP.md](docs/SETUP.md) 复制给 Agent，或运行：
...
│  ┌──────────────┐    ┌──────────────────────────────┐   │
│  │ Skills       │    │ MCP: vidau-geo               │   │
│  │ (this repo)  │───▶│ https://geo.vidau.ai/mcp     │   │
│  │ orchestration│    │ https://geo.vidau.ai/mcp     │   │
│  └──────────────┘    └──────────────┬───────────────┘   │
└─────────────────────────────────────┼───────────────────┘
                                      │
                                      ▼
...
- **MCP** = live API connection (34 tools: audits, brands, compose, publish, automation, …)
- Skill install is **public** (no API key). MCP uses the `vidau-geo` block above.
...
Vidau-Geo-Agent/
├── README.md
├── docs/SETUP.md              # One-click MCP + Skills setup prompt
├── manifest.json              # Skill pack metadata & install URLs
├── package.json               # pnpm skills:install
├── scripts/install-skills.mjs # Local / CDN / GitHub API installer
...
| Item | Required |
|------|----------|
| VidAU GEO account | Yes (for MCP calls) |
...
| Skills user guide | [SKILLS_GUIDE.md](./SKILLS_GUIDE.md) · [Help Center](https://geo.vidau.ai/en/help/agent-skills) |
